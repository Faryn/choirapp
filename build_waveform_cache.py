...
def ffmpeg_waveform(path: Path):
    # mono 8-bit PCM summary for cheap waveform extraction
    result = subprocess.run(
        [
...
def main() -> None:
    ...
    for path in sorted(root.rglob('*')):
        ...
        try:
            ...
            song_name = path.relative_to(root).parts[0]
            by_song.setdefault(song_name, {})[key] = cache[key]
            print('cached', key)
    ...
    for song_name, song_cache in sorted(by_song.items()):
        song_out = split_dir / f'{song_slug(song_name)}.json'
        song_out.write_text(json.dumps(song_cache, ensure_ascii=False, separators=(',', ':')), encoding='utf-8')
    print(f'wrote {len(by_song)} split waveform files to {split_dir}')
